...
class LibvirtGenericVIFDriver(object):
    ...
    def _get_virtio_mq_settings(self, image_meta, flavor):
        ...
        if img_props.get('hw_vif_multiqueue_enabled'):
            ...
            if max_tap_queues:
                vhost_queues = max_tap_queues if flavor.vcpus > max_tap_queues else flavor.vcpus
            else:
    ...
    def _get_max_tap_queues(self):
        # NOTE(kengo.sakai): In kernels prior to 4.0,
        # the number of queues on a tap interface is limited to 8.
        # From 4.0, the number is 256.
